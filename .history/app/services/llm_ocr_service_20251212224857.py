...
"""
        ...
        try:
            ...
            # --- NEW: remove trailing newline for vergi_no ---
            if isinstance(result.get("vergi_no"), str):
                result["vergi_no"] = result["vergi_no"].strip()

            # Fallback regex if JSON missing field
            if "vergi_no" not in result or result["vergi_no"] in (None, ""):
