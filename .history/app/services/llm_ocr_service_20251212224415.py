...
"""
        ...
        try:
            ...
            # ---- NEW: strip ALL string values (fix the \n) ----
            ...
            if "vergi_no" not in parsed or parsed["vergi_no"] in [None, "", "null"]:
                vergi_no = re.search(r"\b(\d[\d\s]{9,})\b", raw_text)
                if vergi_no:
                    parsed["vergi_no"] = vergi_no.group(1).replace(" ", "")
                else:
                    parsed["vergi_no"] = None
